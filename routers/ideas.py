...
@router.get("/")
async def get_ideas(db: Session = Depends(get_db), limit: Optional[int] = 99999, skip: Optional[int] = 0, search: Optional[str] = ""):
    # cursor.execute("""SELECT * FROM ideas """)
    # ideas = cursor.fetchall()
    # print(ideas)
    ideas = db.query(models.Ideas).order_by(
        models.Ideas.id).filter(models.Ideas.shortname.contains(search)).limit(limit).offset(skip).all()
    return ideas
